  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2 text-sm">Sign in to your ExpenseTrack account</p>
      </div>

      {/* Social login */}
      <div className="mb-6">
        <button className="flex w-full items-center justify-center gap-2 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] rounded-xl py-2.5 text-sm text-slate-300 font-medium transition-all">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link to="#" className="text-emerald-400 text-xs hover:text-emerald-300 transition-colors">Forgot password?</Link>
          </div>
          <div className="relative">
        ...
        <div className="flex items-center gap-2">
          <input type="checkbox" id="remember" defaultChecked className="w-4 h-4 rounded accent-emerald-500" />
          <label htmlFor="remember" className="text-slate-400 text-sm">Remember me for 30 days</label>
        </div>
      ...
      <p className="text-slate-500 text-sm text-center mt-6">
        ...
        <Link to="/register" className="text-emerald-400 hover:text-emerald-300 font-semibold transition-colors">Create one free</Link>
      </p>
    </div>
  );
}
